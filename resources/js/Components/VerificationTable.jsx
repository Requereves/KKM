...
const VerificationTable = ({ items, onApprove, onReject, onView, onViewAll, lang = 'id' }) => {
  const t = translations[lang];
  
  // State untuk menangani modal konfirmasi aksi (Approve/Reject)
  const [confirmingAction, setConfirmingAction] = useState(null);

  // Menjalankan aksi setelah user klik tombol di modal konfirmasi
  const executeAction = () => {
    if (!confirmingAction) return;
    
    if (confirmingAction.type === 'approve') {
      onApprove(confirmingAction.id);
    } else {
      onReject(confirmingAction.id);
    }
    
    // Tutup modal setelah aksi dieksekusi
    setConfirmingAction(null);
  };

  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col overflow-hidden transition-colors h-full">
      ...
      <div className="p-5 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
        ...
        <button 
          onClick={onViewAll}
          className="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300 font-medium cursor-pointer"
        >
          {t.view_all}
        </button>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
            {items && items.length > 0 ? (
              items.map((item) => {
                // Fallback Kunci Data (Fail-Safe)
                const displayName = item.student_name || item.name || item.student || "Unknown Student";
                const displayTitle = item.title || item.document_title || item.documentTitle || "Untitled";
                  ...
                  <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-medium ${
                        displayCategory.toLowerCase() === 'sertifikat' || displayCategory.toLowerCase() === 'certificate'
      ...
      {/* --- FOOTER CARD --- */}
      {/* Posisinya di sini agar menyatu dengan tabel (seperti footer) */}
      <div className="p-4 border-t border-slate-100 dark:border-slate-800 text-center bg-white dark:bg-slate-900">
        <button
          onClick={onViewAll}
          className="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 hover:underline inline-flex items-center gap-1 transition-colors"
        >
